...
class TestComputeRentBenchmarks:
    @pytest.mark.asyncio
    async def test_basic_median(self, storage: PropertyStorage) -> None:
        """With 3+ properties, benchmarks should be computed."""
        for i, price in enumerate([1800, 1900, 2000], start=1):
            prop = Property(
                source=PropertySource.OPENRENT,
                source_id=str(i),
        ...
        count = await storage.compute_rent_benchmarks()
        assert count >= 1
        benchmark = await storage.get_rent_benchmark("E8", 1)
        assert benchmark is not None
        assert benchmark["median_rent"] == 1900
        assert benchmark["sample_count"] == 3

    @pytest.mark.asyncio
    async def test_min_sample_requirement(self, storage: PropertyStorage) -> None:
        """Fewer than 3 properties should not produce benchmarks."""
        for i, price in enumerate([1800, 1900], start=1):
            prop = Property(
                source=PropertySource.OPENRENT,
                source_id=str(i),
                url=HttpUrl(f"https://openrent.com/{i}"),
    ...
    @pytest.mark.asyncio
    async def test_recompute_replaces(self, storage: PropertyStorage) -> None:
        """Recomputing should replace old benchmarks."""
        for i, price in enumerate([1800, 1900, 2000], start=1):
            prop = Property(
                source=PropertySource.OPENRENT,
                source_id=str(i),
        ...
        b1 = await storage.get_rent_benchmark("E8", 1)
        assert b1 is not None

        # Add a 4th property and recompute
        prop4 = Property(
            source=PropertySource.OPENRENT,
            source_id="4",
            url=HttpUrl("https://openrent.com/4"),
            title="Flat 4",
            price_pcm=2200,
            bedrooms=1,
            address="4 Street",
        ...
        merged4 = MergedProperty(
            canonical=prop4,
            sources=(PropertySource.OPENRENT,),
            source_urls={PropertySource.OPENRENT: prop4.url},
            min_price=2200,
            max_price=2200,
        )
        await storage.save_merged_property(merged4)
        await storage.mark_notified(merged4.unique_id)
        await storage.compute_rent_benchmarks()
        b2 = await storage.get_rent_benchmark("E8", 1)
        assert b2 is not None
        # Median of [1800, 1900, 2000, 2200] = 1950
        assert b2["median_rent"] == 1950
