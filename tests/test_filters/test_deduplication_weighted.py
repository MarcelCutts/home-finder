...
class TestMatchScore:
    ...
    def test_two_signals_at_threshold(self) -> None:
        score = MatchScore(image_hash=40, price=15)  # 55 points, 2 signals
...
class TestCalculateMatchScore:
    ...
    def test_coordinates_match(self, base_property: Property) -> None:
        prop2 = base_property.model_copy(
            update={
                "source": PropertySource.ZOOPLA,
                "source_id": "456",
    ...
    def test_price_mismatch_reduces_score(self, base_property: Property) -> None:
        prop2 = base_property.model_copy(
            update={
                "source": PropertySource.ZOOPLA,
                "source_id": "456",
                "price_pcm": 1700,  # Outside 3% tolerance
            }
        )
        score = calculate_match_score(base_property, prop2)
...
class TestMatchThresholdConstants:
    ...
    def test_threshold_value(self) -> None:
        assert MATCH_THRESHOLD == 55

    def test_minimum_signals_value(self) -> None:
        assert MINIMUM_SIGNALS == 2

    def test_postcode_plus_price_meets_threshold(self) -> None:
        """Full postcode (40) + price (15) = 55 meets threshold."""
        score = MatchScore(full_postcode=40, price=15)
        assert score.total == MATCH_THRESHOLD
        assert score.signal_count >= MINIMUM_SIGNALS
        assert score.is_match is True

    def test_coords_plus_price_meets_threshold(self) -> None:
        """Coordinates (40) + price (15) = 55 meets threshold."""
        score = MatchScore(coordinates=40, price=15)
        assert score.total == MATCH_THRESHOLD
        assert score.is_match is True
